...
pub fn annealing_easy<S: State>(problem: &mut Box<dyn Metaheuristics<S>>, runtime: Duration) -> S {
    ...
    loop {
		...
		if now_time > runtime { break; }
		let now_as_milli = now_time.as_millis() as f64;
        let mut next_candidate = problem.neighbor(&best_candidate);

		if problem.evaluate(&mut next_candidate) > problem.evaluate(&mut current_candidate) || rng.gen::<f64>() < probability(now_as_milli / runtime_as_milli) {
			current_candidate = next_candidate;
