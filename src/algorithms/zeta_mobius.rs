use crate::utils::algebraic_traits::{Group, ComGroup, Ring, Field};
...
mod tests {
	...
	where F: Fn(usize, usize) -> Option<usize> {
		...
		for i in 0..n {
			for j in 0..n {
				if let Some(k) = func(i, j) { c[k] += a[i] * b[j]; }
			}
		}
		c
	}
	...
	fn test_xor_conv() {
		let a = vec![1, 2, 3, 4, 5, 6, 7, 8];
		let b = vec![9, 10, 11, 12, 13, 14, 15, 16];
		let c = xor_convolution(&a, &b).into_iter().map(|x| x / 8).collect::<Vec<i32>>();
		let d = brute_conv(&a, &b, |i, j| Some(i^j));
		assert_eq!(c, d);
	}
