...
use crate::utils::fp::{self, Fp, Mod};
mod arith;
mod fourier;
...
macro_rules! impl_fourier_and_convolution {
    ($(($mod: ident, $Fp: ty, $prim_root: expr, $e: expr),)*) => {$(
        mod $mod {
            use super::{fp, Convolution, Fourier};
...
pub fn integer_convolution(a: &[u32], b: &[u32]) -> Vec<u128> {
    type Fp1 = fp::F998244353;
    type Fp2 = fp::F1012924417;
    type Fp3 = fp::F924844033;
...
mod test {
        ...
        Fp<M>: Convolution,
    {
        ...
        for _ in 0..100 {
            ...
            let a: Vec<Fp<M>> = repeat_with(|| Fp::<M>::new(rng.gen_range(0..Fp::<M>::P)))
                .take(n)
                .collect();
            let b: Vec<Fp<M>> = repeat_with(|| Fp::new(rng.gen_range(0..Fp::<M>::P)))
                .take(m)
                .collect();
    ...
    fn test_integer_convolution_power_of_two() {
        ...
        for _ in 0..100 {
            let n = rng.gen_range(1..100);
            let m = rng.gen_range(1..100);
            let a: Vec<u32> = repeat_with(|| rng.gen_range(0..std::u32::MAX))
                .take(n)
                .collect();
            let b: Vec<u32> = repeat_with(|| rng.gen_range(0..std::u32::MAX))
                .take(m)
                .collect();
    ...
    fn test_anymod_convolution() {
        ...
        for _ in 0..100 {
            let n = rng.gen_range(1..100);
            let m = rng.gen_range(1..100);
            let a: Vec<Fp> = repeat_with(|| Fp::new(rng.gen_range(0..Fp::P)))
                .take(n)
                .collect();
            let b: Vec<Fp> = repeat_with(|| Fp::new(rng.gen_range(0..Fp::P)))
                .take(m)
                .collect();
